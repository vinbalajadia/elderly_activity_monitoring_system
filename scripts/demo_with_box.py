...
while True:
    ...
    # 1. AI PREDICTION (On Whole Frame - Stable)
    ...
    # We use a lower scale/stride for speed since AI is already done
    ...
    if stable_label != "Waiting...":
        ...
        if largest_box is not None:
            ...
            # Text at Top-Left of Box (with black background for readability)
            (text_w, text_h), _ = cv2.getTextSize(text_display, cv2.FONT_HERSHEY_SIMPLEX, 0.8, 2)
            cv2.rectangle(frame, (x, y - 30), (x + text_w, y), color, -1)
            cv2.putText(frame, text_display, (x, y - 6), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2)
        else:
            # Fallback: No box found, draw at top left of screen
            cv2.rectangle(frame, (0, 0), (640, 50), (0, 0, 0), -1)
            cv2.putText(frame, text_display, (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
